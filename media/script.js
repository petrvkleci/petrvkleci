const mediaData = {
    organization: {
        ...
        articles: [
        {
            ...
            image: "img/housenka-vs-zazvor-2027.png",
            content: [
                "Už 1. září 2027 proběhne zápas mezi Markem Housenkou a Antonínem Zázvorem.",
        ...
        },
        {
            ...
            date: "2026-05-22",
            tag: "Tisková zpráva",
            title: "Antonín Zázvor podepisuje smlouvu s Petr v Kleci",
            image: "img/logo-transparent.png",
            content: [
                "Antonín Zázvor oznámil podpis smlouvy s organizací Petr v Kleci.",
                "Smlouva je platná na dva roky a zahrnuje možnosti zápasů.",
                "Fanoušci jsou nadšeni a očekávají, že Antonín Zázvor bude vystupovat v zápasech."
            ]
        },
        {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function excerptOf(article) {
        const text = (article.content && article.content[0]) || '';
    ...
    function showArticle(slug) {
        ...
        document.getElementById('detailGalleryWrap').innerHTML = (article.gallery && article.gallery.length)
            ? `<div class="gallery-grid">${article.gallery.map(src => `<a href="${src}" target="_blank" rel="noopener"><img src="${src}" alt=""></a>`).join('')}</div>`
            : '';

        const embedUrl = article.video ? youtubeEmbedUrl(article.video) : null;
        document.getElementById('detailVideoWrap').innerHTML = embedUrl
            ? `<div class="video-embed"><iframe src="${embedUrl}" title="Video" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>`
            : '';

        document.getElementById('detailLinkWrap').innerHTML = article.linkUrl
            ? `<a href="${article.linkUrl}" target="_blank" rel="noopener" class="btn btn-primary">${article.linkLabel || 'Zdroj / více informací →'}</a>`
            : '';
    ...
    const itemListLd = {
        ...
        "itemListElement": sorted.map((a, i) => ({
            ...
            }, a.image ? { "image": location.origin + a.image.replace('..', '') } : {})
        }))
    };
